	...
	public static boolean readExcel(String destination, String fields, String sourceFile, int idFile) {
		...
		try {
			...
			while (rowIterator.hasNext()) {
				...
				//kiểm tra xem đủ số field hay không
				if (arFiels.length == last) {
...
//				} 
				...
				while (cellIterator.hasNext()) {
					...
					case NUMERIC:
						...
						statement2.setString(columnIndex+1, value);
						break;
					case STRING:
						String valueString = nextCell.getStringCellValue();
						statement2.setString(columnIndex+1, valueString);
						break;
					default:
						statement2.setString(columnIndex+1, "Null");
						break;
					}
				}
				//kiểm tra xem lấy đủ dữ liệu hay k
				if (count == last) {
					statement2.execute();
				}
			}
